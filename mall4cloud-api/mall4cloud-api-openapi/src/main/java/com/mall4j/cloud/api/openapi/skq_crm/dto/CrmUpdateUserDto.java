...
/**
 * @description: 会员信息更新（crm端）接口请求参数
 * @date 2021/12/23 22:10
 */
public class CrmUpdateUserDto implements Serializable, IDataCheck {
    ...
    public String toString() {
        return "CrmUpdateUserDto:{" +
                ...
                ", current_valid_point='" + current_valid_point + '\'' +
    ...
    public CrmResponse check() {
        ...
        if (StringUtils.isBlank(vipcode)) {
            return CrmResponse.fail(CrmResponseEnum.HTTP_MESSAGE_NOT_READABLE.value(), "crm端会员标识不能为空");
        }
        if (gender != null && (!"F".equals(gender) || !"M".equals(gender))) {
            return CrmResponse.fail(CrmResponseEnum.HTTP_MESSAGE_NOT_READABLE.value(), "性别参数格式不正确");
        }

        if (StringUtils.isNotBlank(birthday)) {
            return CrmResponse.fail(CrmResponseEnum.HTTP_MESSAGE_NOT_READABLE.value(), "生日参数格式不正确，正确格式为：yyyy-MM-dd");
        }
