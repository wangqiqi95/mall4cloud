...
import com.mall4j.cloud.common.exception.RateLimitException;
...
public class DefaultExceptionHandlerConfig {
	...
	public ResponseEntity<ServerResponseEntity<?>> luckExceptionHandler(LuckException e) throws TransactionException{
		logger.warn("mall4cloudExceptionHandler", e);
    ...
    public ResponseEntity<ServerResponseEntity<Object>> rateLimitException(Exception e) throws TransactionException {
        logger.error("exceptionHandler", e);
        logger.info("RootContext.getXID(): " + RootContext.getXID());
        if (StrUtil.isNotBlank(RootContext.getXID())) {
            GlobalTransactionContext.reload(RootContext.getXID()).rollback();
        }
        return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body(ServerResponseEntity.fail(ResponseEnum.RATELIMITEXCEPTION));
    }

	@ExceptionHandler(HystrixRuntimeException.class)
	public ResponseEntity<ServerResponseEntity<Object>> hystrixRuntimeException(Exception e) throws TransactionException {
		logger.error("hystrixRuntimeException", e);
		logger.info("RootContext.getXID(): " + RootContext.getXID());
		if (StrUtil.isNotBlank(RootContext.getXID())) {
			GlobalTransactionContext.reload(RootContext.getXID()).rollback();
		}
		return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(ServerResponseEntity.fail(ResponseEnum.RATELIMITEXCEPTION));
	}
}
