...
@Slf4j
@RestController("platformCrmAPiController")
@RequestMapping("/p/crm/api")
@Api(tags = "后台-数云接口")
public class CrmAPiController {
    ...
    @ApiOperation(value = "积分明细查询", notes = "积分明细查询")
    ...
    @GetMapping("/editageUserisRegister")
    @ApiOperation(value = "用户数据查询-是否注册", notes = "用户数据查询-是否注册")
    public ServerResponseEntity<Boolean> editageUserisRegister(@RequestParam(value = "unionId",required = true) String unionId,
                                                               @RequestParam(value = "active",required = false,defaultValue = "") String active,
                                                               @RequestParam(value = "mobileNumber",required = false,defaultValue = "") String mobileNumber) {
        ServerResponseEntity<CrmResult<JSONArray>> crmResult=crmService.editageUser(unionId,active,mobileNumber);
        log.info("获取数据用户数据：{}", JSON.toJSONString(crmResult));
        if(Objects.nonNull(crmResult)){
            JSONArray jsonArray=crmResult.getData().getResult();
