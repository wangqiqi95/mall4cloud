...
public class ShopCartFeignController implements ShopCartFeignClient {
    ...
    public ServerResponseEntity<List<ShopCartItemVO>> getCheckedShopCartItems(Long storeId) {
        List<ShopCartItemVO> checkedShopCartItems = shopCartService.getCheckedShopCartItems(storeId);
        if (CollectionUtil.isNotEmpty(checkedShopCartItems)) {
            for (ShopCartItemVO shopCartItem : checkedShopCartItems) {
                ...
                shopCartItem.setMarketTotalPrice(shopCartItem.getCount() * shopCartItem.getMarketPriceFee());
