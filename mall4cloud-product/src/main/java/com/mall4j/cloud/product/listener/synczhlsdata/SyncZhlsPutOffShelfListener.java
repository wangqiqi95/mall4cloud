...
@RocketMQMessageListener(topic = RocketMqConstant.SYNC_ZHLS_PRODUCT_TOPIC,
        consumerGroup = "GID_" + RocketMqConstant.SYNC_ZHLS_PUT_OFF_SHELF_GROUP,
        selectorExpression = RocketMqConstant.SYNC_ZHLS_PUT_OFF_SHELF_TAG
)
public class SyncZhlsPutOffShelfListener implements RocketMQListener<List<Long>> {
    ...
    public void onMessage(List<Long> spuIds) {
        log.info("同步有数监听商品下架mq入参:{}", JSON.toJSONString(spuIds));
        ...
        zhlsCommodityService.skuInfoUpdateStatus(skuIds,SpuStatus.OFF_SHELF.value());
    }
}
