package com.mall4j.cloud.biz.util.crm;

import cn.hutool.http.HttpUtil;
import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.mall4j.cloud.biz.config.CrmParams;
...
public class CrmClients {
	...
	public String postCrm(String uri, String bodyStr) {
		...
		try {
			...
			headerMap = CrmUtils.generateHeader(builder.toString(),crmParamsCopy.getCallerService(),crmParamsCopy.getServiceSecret());
...
//			result = HttpUtils.doPost(builder.toString(), bodyStr);
			...
			e.printStackTrace();
			logger.error("调用接口异常，url为:{}, url参数为:{}, json参数为:{}, 异常信息:{}, 共耗时: {}",builder.toString(), builder.toString(), bodyStr, e,
					System.currentTimeMillis() - start);
		}finally {
			logger.info("调用接口结束，url为:{}, url参数为:{}, json参数为:{}, 请求响应为:{}, 共耗时: {}", builder.toString(), JSON.toJSONString(headerMap), bodyStr, result,
						System.currentTimeMillis() - start);
		}
		return result;
	}
